...
$$\begin{aligned}y_{i} = \beta_ix_i + \epsilon_{i}\\with\ y = log(hidden), x_i = volatility\_rank, \epsilon_{i} = error\end{aligned}$$
...
fit_sec7 |> 
  ...
  labs(title = "Posterior for Hidden Orders per Stock",
       subtitle = "Low and medium volatility stocks experience a higher percentage of hidden orders", 
       x = "Hidden Orders (Log)",
       y = "Probability",
       fill = "Volatility")
```
