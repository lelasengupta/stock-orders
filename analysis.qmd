...
security <- read_csv("q1_2024_all.csv") |> 
  ...
  select(security, ticker, volatility_rank, hidden) |>
  drop_na()

security1 <- security %>% sample_n(10000)
```
...
brm(formula = hidden ~ volatility_rank,
    data = security1,
    family = gaussian(),
    silent = 2,
    refresh = 0,
...
covariates: volatility ranking (1-10), security type, could do another one with market cap rank
